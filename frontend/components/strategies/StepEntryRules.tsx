...
type Props = {
  value: StrategySchemaV1['entry'];
  onChange: (v: StrategySchemaV1['entry']) => void;
};
...
const StepEntryRules: React.FC<Props> = ({ value, onChange }) => {
  const [entry, setEntry] = React.useState<EntryRules>(value);
  const [errors, setErrors] = React.useState<string[]>([]);

  React.useEffect(() => setEntry(value), [value]);

  function commit(next: EntryRules) {
    setEntry(next);
    onChange(next);
  ...
  function addGroup() {
    commit({ ...entry, groups: [...(entry.groups || []), newGroup()] });
  }

  function removeGroup(idx: number) {
    const groups = (entry.groups || []).filter((_, i) => i !== idx);
    commit({ ...entry, groups });
  }
  ...
  function updateGroupOp(gIdx: number, op: 'AND' | 'OR') {
    const groups = (entry.groups || []).map((g, i) => (i === gIdx ? { ...g, op } : g));
    commit({ ...entry, groups });
  }
  ...
  return (
    <Card>
      <CardHeader>
        <h2 className="text-base font-semibold text-gray-900">Entry Rules</h2>
        <p className="mt-1 text-sm text-gray-600">Define side and condition groups.</p>
      </CardHeader>
      <CardContent>
...
export default React.memo(StepEntryRules);